...
若原运行的补题活动已停止，但这次运行仍有有效的已领题，可用原运行码执行 `dradar run --plan <运行码> --server <网页 API 地址> --held-only --concurrency 1 --json`。服务端会重新核验这台设备的权限；此模式只处理原批已领题，不重新启动补题或领取新题。已提交题目须经服务端精确状态核验后才从本地未完成边界中结算；无法核验时会在启动模型前停止。直接重试内部 `fleet add` 不会重新授权已停止的运行设备。
...
如在 macOS 使用自定的 `DRADAR_HOME=/tmp/...`，改用同一目录的物理路径
`/private/tmp/...`；`doctor` 会在领题前提示路径问题，直接运行也会在启动
付费模型前停止，保留原任务供按原运行说明恢复。
