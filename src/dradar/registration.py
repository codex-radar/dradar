...
class RegistrationWindow:
    ...
    def check(self):
        ...
        if (self.telemetry is not None and
            ...
            raise ApiError("worker registration stopped")
        if time.monotonic() >= self.deadline:
            raise ApiError("worker registration budget expired")
        if not self.alive():
            raise ApiError("worker exited during registration")
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                if not isinstance(result, dict):
                    raise ApiError("invalid registration response")
                return result
            except _TRANSIENT as exc:
                if attempt + 1 == attempts:
                    raise ApiError("registration transport failed") from exc
    ...
    def bind(self, api, telemetry, assignment):
        ...
        try:
            ...
            if response.get("ok") is not True:
                raise ApiError("assignment start was not confirmed")
        ...
        with _checked_lock(t._send_lock, self.check):
            ...
            async with self._client() as client:
                ...
                if response.get("stop_requested") is True:
                    ...
                    raise ApiError("server requested worker stop")
                if response.get("accepted") is not True:
                    diagnostic["registration_result"] = "heartbeat_rejected"
                    raise ApiError("worker heartbeat was not accepted")
                if response.get("batch_id", batch) != batch:
                    raise ApiError("worker heartbeat batch changed")
                self.check()
                ...
                if (not isinstance(ids, list) or not all(recorder._valid_event_id(i) for i in ids)
                    ...
                    raise ApiError("exact worker event was not acknowledged")
    ...
    def _start_request(self, method, path, **kwargs):
        ...
        if path == "/api/v1/assignment/started":
            if self.deadline - time.monotonic() < 4.0:
                raise ApiError("insufficient budget for worker start handoff")
    ...
    async def _close_fence(self):
        ...
        async with self._client() as client:
            ...
            if result.get("ok") is not True:
                raise ApiError("registration close fence unconfirmed")
            self._fenced = True
            self.assignment.pop("_registration_start_uncertain", None)
