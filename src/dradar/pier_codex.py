...
class CodexRegistered(Codex):
    def verify_gpt6_subscription_auth(self, source):
        ...
        if any((os.environ.get(key) or extra_env.get(key)) for key in ("OPENAI_API_KEY", "CODEX_API_KEY")):
            raise RuntimeError("GPT-6 trial requires ChatGPT subscription authentication")
        if not source:
            raise RuntimeError("GPT-6 trial requires ChatGPT subscription authentication")
        try:
            data = credential_json(read_private_credential(Path(source)))
    ...
    async def run(self, instruction, environment, context):
        ...
        if gpt6:
            self.verify_gpt6_subscription_auth(source)
