...
The console entry, `python -m dradar.cli`, and newly built PYZ share
`dradar.launcher`. This is independent of the selected harness: Kimi, DSH,
Codex, Claude Code and other supported adapters all use the same policy.
There is no per-update user confirmation and no dependency on a server text
notice. On launch,
...
and its children (verified PYZ or installed-source workers/Fleet processes)
hold independent invocation locks for their whole run,
...
discovery, not the parent's artifact verification. Source children retain the
installed package through the module entry and carry a separate internal
`DRADAR_OTA_SOURCE_CHILD` marker; they also bypass selection and periodic
discovery, while taking their own activity lease. The marker is consumed before
command execution and regenerated by the shared child process helper for
grandchildren. Source workers are not described as signed verified children. An internal environment flag
...
launch method and whether this invocation participates in the OTA launcher
protocol. `launch_method=launcher` includes source children protected by their
parent launcher; it does not prove that this individual process owns a discovery
timer. Likewise `unknown` on an older source child does not prove that its
parent was not checking updates. It does not
...
## Remaining rollout and lifecycle boundaries

This entry-point fix does not retrofit a running old process. Existing users
need to reach a normal launcher restart before it can apply. It does not add a
Fleet/refill drain-and-restart protocol: a continuously active supervisor still
blocks activation, including activation of an older waiting candidate. The
candidate is preserved rather than deleted or silently replaced. Bounded
campaigns can activate after all work and uploads finish; unlimited campaigns
need a separately validated lifecycle handoff to guarantee eventual activation.
Do not treat this fix as proof that any specific online user has recovered.
