...
def build_artifact(destination):
    ...
    module._build_zipapp(ROOT, destination, version=__version__, sequence=224,
...
def test_source_launcher_matches_zip_capabilities(tmp_path):
    ...
    assert source['version'] == zipped['version'] == '0.5.224'
    from dradar import providers as p
...
def test_public_launcher_selects_signed_gpt6_ota_payload(tmp_path, monkeypatch):
    ...
    document['release_id'] = 'dradar-cli-0.5.224-gpt6-fixture'
    document['version'] = '0.5.224'
    document['sequence'] = 5224
    ...
    # Model an installed public 0.5.223 launcher discovering the new release.
    monkeypatch.setattr(discovery, '__version__', '0.5.223')
