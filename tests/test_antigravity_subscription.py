...
def test_antigravity_loop_breaker_triggers_on_consecutive_repeats(tmp_path: Path) -> None:
    script = _antigravity_adapter_constant("ANTIGRAVITY_LOOP_BREAKER_SCRIPT")
    tool_events = [
        {
            "event": "step_update",
            "step_update": {
                ...
                "step_type": "tool",
                "tool_name": "view_file",
                "tool_info": {"parameters": {"AbsolutePath": "/app/stuck.py"}},
            },
        }
        for i in range(5)
    ]
    tool_events.append({
        "event": "result",
        "result": {"status": "INTERRUPTED", "response": "Interrupted by signal"},
    })
    input_text = "\n".join(json.dumps(ev) for ev in tool_events) + "\n"
    result = subprocess.run(
        [sys.executable, "-c", script],
    ...
    assert result.returncode == 0
    assert result.stdout == input_text
    assert "Deadlock detected: 5 consecutive identical calls to read-only tool 'view_file'" in result.stderr
    assert "Triggering interrupt." in result.stderr


def test_antigravity_loop_breaker_configurable_threshold() -> None:
    script = _antigravity_adapter_constant("ANTIGRAVITY_LOOP_BREAKER_SCRIPT")
    tool_events = [
        {
            "event": "step_update",
            "step_update": {
                ...
                "step_type": "tool",
                "tool_name": "list_dir",
                "tool_info": {"parameters": {"DirectoryPath": "/app/src"}},
            },
        }
        for i in range(3)
    ]
    input_text = "\n".join(json.dumps(ev) for ev in tool_events) + "\n"
    env = dict(os.environ, DRADAR_LOOP_BREAKER_MAX_REPEATS="3")
    result = subprocess.run(
        [sys.executable, "-c", script],
        input=input_text,
        text=True,
    ...
    assert result.returncode == 0
    assert result.stdout == input_text
    assert "Deadlock detected: 3 consecutive identical calls to read-only tool 'list_dir'" in result.stderr


def test_antigravity_adapter_wires_loop_breaker_in_install_and_run() -> None:
    adapter_source = Path(providers.__file__).with_name("pier_antigravity.py").read_text()
    assert "ANTIGRAVITY_LOOP_BREAKER_SCRIPT" in adapter_source
    assert "/opt/antigravity-runtime/bin/loop_breaker.py" in adapter_source
    assert "chmod 0755 /opt/antigravity-runtime/bin/loop_breaker.py" in adapter_source
    assert "dradar_breaker=/tmp/loop_breaker.py" in adapter_source
    assert 'python3 -u "$dradar_breaker"' in adapter_source
    assert "/tmp/dradar-loop-breaker-triggered" in adapter_source
    assert 'cache_key=f"dradar-antigravity-{version}-linux-runtime-v2"' in adapter_source
